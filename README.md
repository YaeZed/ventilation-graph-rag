...
- Qwen3.5-Omni 图片理解：单图先初步观察并列出不确定概念，再检索通风概念定义，最后结合概念卡片完成场景、字段和风险分析；多图会逐张观察、合并概念并输出跨图关联发现。
- Django REST/SSE API，包含开发期账号、用户资料和会话同步接口。
- Vue3 + TypeScript + Pinia 前端，支持多图片上传、传感器数据录入/CSV 粘贴、流式回复、Agent 步骤展示、Markdown 渲染、多会话管理、搜索、归档、登录/注册、团队空间、统计、导出和会话级输入草稿隔离。
- 前端用户层本地优先：未登录时使用浏览器 localStorage 保存会话、偏好设置和简易用户身份；登录用户按账号隔离本地缓存并同步到 Django 后端。已有账号登录只加载该账号数据，注册新账号时才迁移游客会话。浏览器本地缓存可保留压缩后的消息图片预览以支撑刷新恢复；远端会话同步会剥离 data URL。登录用户图片优先写入后端 `ConversationAttachment`，原图超过后端限制时会用压缩预览重试上传。
- 账号安全基线已接入：Django CSRF、密码校验器、登录失败限流、session cookie 安全配置和账号安全事件记录。
- `/settings` 支持运行时模型服务配置：可选择 DashScope、OpenAI、Ollama 或自定义 OpenAI-compatible endpoint。访客配置保存在本地，登录用户配置随账号偏好同步；每次聊天请求会携带当前配置并只覆盖该请求的文本/视觉模型调用。公开部署前必须改成用户自带密钥或受控租户密钥，不能让普通用户默认消耗维护者 `.env` 模型密钥。
- 图片/传感器辨识统一在会话窗口完成：上传一张或多张图片、添加传感器数据后，用户描述会与图像/数值一起进入 Qwen3.5-Omni + RAG 融合辨识链路。
...
打开 `http://127.0.0.1:5173`，默认路由会进入 `/chat`。左侧 Gemini 风格侧边栏支持新建/切换/搜索/重命名/归档/删除对话，统计页在 `/stats`，偏好设置和团队管理在 `/settings`，账号入口在 `/login` 和 `/register`。

如需切换模型服务，到 `/settings` 的“模型服务”面板选择预设或填写自定义模型名、endpoint 和 API key。可先点“测试连接”分别验证文本/视觉模型的 OpenAI-compatible `chat.completions` 是否可用，再发起真实对话。文本模型用于路由分析、检索辅助和最终生成；视觉模型用于单图/多图观察与分析。开发态未填写 key 时后端继续使用 `.env` 中的默认 DashScope 配置；公开部署前需要改为用户自带 key 或后端受控租户 key。

图片与传感器辨识无需进入单独页面；在主会话窗口点击 `+` 上传一张或多张图片，点击“数”录入传感器数据或粘贴 CSV，再输入现场描述或检查重点后发送即可。发送后的图片会显示在用户消息中，点击缩略图会在当前页面中央放大预览；多图预览可用左右箭头或键盘方向键切换，长图会限制在弹层边框内。
